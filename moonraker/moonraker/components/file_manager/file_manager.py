...
class InotifyNode:
    ...
    async def complete_file_write(self, file_name: str) -> None:
        ...
        if evt_name is None:
            #logging.info(f"Invalid file write event: {file_name}")#没什么用
            return
...
class MetadataStorage:
    ...
    async def _process_metadata_update(self) -> None:
        stack = inspect.stack()
        logging.info("\033[7m\033[5m" + f'_process_metadata_update called by {stack[1][3]}' + "\033[0m")
        self.server.send_event("server:gcode_response", "start update metadata")
        while self.pending_requests:
            ...
            mevt.set()
        logging.info("\033[7m\033[5m" + "_process_metadata_update finish" + "\033[0m")
        self.server.send_event("server:gcode_response", "update metadata finish")
        self.busy = False
